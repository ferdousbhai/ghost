    ...
    Flickable {
        ...
        Column {
            ...
            Column {
                ...
                Rectangle {
                    width: parent.width
                    height: obsidianCopy.implicitHeight + Theme.pad
                    radius: Theme.radius
                    color: root.statusFill(String(root.snapshot.obsidian.status))
                    border.width: 1
                    border.color: root.statusColor(String(root.snapshot.obsidian.status))
                    ...
                    Column {
                        ...
                        Row {
                            ...
                            Text {
                                width: parent.width - obsidianStatus.width - parent.spacing
                                text: "obsidian-cli"
                                textFormat: Text.PlainText
                                color: Theme.foregroundBright
                                font.family: Theme.fontFamilyMono
                            ...
                            StatusBadge {
                                id: obsidianStatus
                                status: String(root.snapshot.obsidian.status)
                            }
                        }

                        Text {
                            width: parent.width
                            text: String(root.snapshot.obsidian.path)
                            textFormat: Text.PlainText
                            color: Theme.foregroundDim
                            font.family: Theme.fontFamilyMono
                            font.pixelSize: Theme.fontSizeCaption
                        ...
                        Text {
                            width: parent.width
                            visible: text !== ""
                            text: typeof root.snapshot.obsidian.reason === "string"
                                ? root.snapshot.obsidian.reason : ""
                            textFormat: Text.PlainText
                            color: Theme.danger
                            font.family: Theme.fontFamily
                            font.pixelSize: Theme.fontSizeSmall
            ...
            SectionLabel {
                visible: root.snapshot !== null
                label: "Skills · " + root.skills.length
            }

            Repeater {
                model: root.skills
                ResourceRow { required property var modelData; resource: modelData; mcp: false }
            }

            SectionLabel {
                visible: root.snapshot !== null
                label: "MCP servers · " + root.mcpServers.length
            }

            Repeater {
                model: root.mcpServers
                ResourceRow { required property var modelData; resource: modelData; mcp: true }
            }

            Column {
                width: parent.width
                visible: root.snapshot !== null && root.diagnostics.length > 0
                spacing: Theme.gap

                SectionLabel { label: "Skipped diagnostics · " + root.diagnostics.length }
                ...
                Repeater {
                    ...
                    Text {
                        ...
                        text: String(modelData.source) + " · "
                            + (typeof modelData.path === "string" && modelData.path !== ""
                                ? modelData.path + " · " : "") + String(modelData.reason)
                        textFormat: Text.PlainText
                        color: Theme.danger
                        font.family: Theme.fontFamilyMono
                        font.pixelSize: Theme.fontSizeCaption
