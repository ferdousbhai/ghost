...
Singleton {
    ...
    // Background jobs are the conversation-scoped work view.
    ...
    readonly property bool workHasContent: root.workJobs.length > 0
    ...
    function fetchWorkResource(kind: string, force: bool,
        ...
        if (kind !== "jobs" || (!force && (root.workJobsLoading || root.workJobsLoaded))) return;
