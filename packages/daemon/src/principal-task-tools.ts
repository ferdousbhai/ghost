...
export function createPrincipalTaskTools(options: PrincipalTaskToolsOptions): GhostExtensionFactory {
  return (api) => {
    api.registerTool({
      name: "task",
      label: "Delegate task",
      description: "Start one durable asynchronous coding task; retain the returned task id for later task_get, task_send, or task_cancel calls.",
