 ...
 * `GhostPiRuntime` is scoped to the ghost's own `models.json` and `.pi/auth.json`,
 * so "usable" is per-ghost by construction. The
 ...
 * runtime so it always reflects the ghost's current models and credentials.
 */
