...
function readMemory(stdout: string): {
  slugs: string[];
  skipped: Array<{ path: string; reason: string }>;
} {
  const body = JSON.parse(stdout) as {
    memory?: Array<{ slug?: unknown }>;
  ...
  return {
    slugs: body.memory.map((row) => String(row.slug)),
    skipped: body.skipped.map((row) => ({
...
export async function smokeCommand(
  ...
  try {
    env.GHOST_CLAUDE_BINARY = await resolveClaudeCodeExecutable(
      ctx.runtime.env.GHOST_CLAUDE_BINARY?.trim() || "claude",
    );
  } catch {
  ...
  try {
    ...
    if (flagBoolean(parsed, "no-turn")) {
      ...
      if (memory.skipped.length > 0) {
        throw new Error(
          `unreadable memory file: ${memory.skipped.map((row) => `${row.path} (${row.reason})`).join("; ")}`,
        );
      }
      report(
        "memory",
        true,
        memory.slugs.length > 0
          ? `${memory.slugs.length} readable (${memory.slugs.join(", ")})`
          : "none written",
      );
