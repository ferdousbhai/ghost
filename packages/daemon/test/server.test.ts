...
describe("POST /api/ghosts/:name/sessions/:id/recap", () => {
  it("returns a non-persisted Pi side-channel recap and enforces the route boundary", async () => {
    const base = await serve([
    ...
    expect((await fetch(
      `${base}/api/ghosts/casper/sessions/${encodeURIComponent("claude-code:conv-recap")}/recap`,
      { method: "POST", headers: { "content-type": "application/json" }, body: "{}" },
    )).status).toBe(409);
  });
});
