...
function authorityFrom(revalidate: TaskBindingAuthority["revalidate"]): TaskBindingAuthority {
  return {
    revalidate,
...
function trackedStore(home: string): TaskStore {
...
async function eventually(
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    try {
      ...
      if (changed.code !== "unsafe_task_record" || changed.message !== "The task record changed.")
        throw error;
    }
...
async function fixture(
  ...
  const controller = new TaskController(
    ...
    authorityFrom(bindingAuthority.revalidate),
    new FakeNativeTaskScopeManager(),
  );
  await controller.initialize();
  return { home, store, controller };
}
function runtime(
  options: { force?: () => Promise<void>; start?: (context: TaskAdapterContext) => void } = {},
) {
  const result = deferred<string>();
  const quiet = deferred<void>();
  let context!: TaskAdapterContext;
  const adapter: TaskAdapter = {
    async start(_input, next) {
      context = next;
      next.register({
        force:
          options.force ??
...
function record(state: TaskRecord["state"]): TaskRecord {
  ...
  return {
    ...
    result: state === "completed" ? "done" : null,
    resultTruncated: false,
    error:
      state === "failed" || state === "interrupted"
        ? { code: "failed", message: "Safe failure." }
        : null,
  };
}
...
describe("durable task foundation", () => {
  it("requires initialization and revalidates the exact admitted binding before spawn", async () => {
    ...
    const admitted: TaskAdapter = {
      ...native.adapter,
      async start(input, context) {
        received = input;
        return native.adapter.start(input, context);
      },
    };
    const controller = new TaskController(
      store,
      new Map([
        ["native", admitted],
        ["claude-code", admitted],
      ]),
      authority,
      new FakeNativeTaskScopeManager(),
    );
    await expect(start(controller)).rejects.toMatchObject({ code: "tasks_uninitialized" });
    await controller.initialize();
    ...
    expect(await readFile(new URL("../src/tasks.ts", import.meta.url), "utf8")).not.toMatch(
      /node:child_process|\bgit\b/iu,
    );
  });
  ...
  it("starts independent tasks concurrently without a global limit", async () => {
    const executions: Array<{
      ...
      quiet: ReturnType<typeof deferred<void>>;
    }> = [];
    const { store, controller } = await fixture({
      async start(_input, context) {
        const result = deferred<string>();
        const quiet = deferred<void>();
        executions.push({ result, quiet });
        context.register({
          async force() {
            quiet.resolve();
            result.resolve("");
    ...
    await eventually(store, first.id, "running");
    await eventually(store, second.id, "running");
    expect(executions).toHaveLength(2);
    for (const execution of executions) {
      execution.result.resolve("done");
      execution.quiet.resolve();
    }
  });
    ...
    async (_label, names, replacementIndex, spawnIndex) => {
      ...
      const adapter: TaskAdapter = {
        async start(input, context) {
          ...
          await context.launchNative(executables, (spawn) =>
            spawn({
  ...
  it("keeps follow-up and cancellation within the qualified parent", async () => {
    ...
    await expect(controller.followUp(task.id, "foreign", foreign)).rejects.toMatchObject({
      code: "task_not_found",
      status: 404,
    });
    await expect(controller.cancel(task.id, foreign)).rejects.toMatchObject({
      code: "task_not_found",
      status: 404,
    });
    expect((await store.read(task.id)).state).toBe("running");
    await expect(controller.followUp(task.id, " \n\t ", parent)).rejects.toMatchObject({
      code: "invalid_task",
      status: 400,
    });
  ...
  it("registers admission before a blocked durable write and lets shutdown fence it", async () => {
    ...
    const store = new DelayedStore(home);
    stores.push(store);
    const native = runtime();
    const controller = new TaskController(
      store,
    ...
    await controller.initialize();
    staleList = true;
    const admission = start(controller);
    const shutdown = controller.beginShutdown();
    gate.resolve();
    await listEntered.promise;
    listRelease.resolve();
    await expect(admission).rejects.toMatchObject({ code: "tasks_shutting_down" });
    await shutdown;
    staleList = false;
    expect(spawned).toBe(false);
    expect((await store.list())[0]?.state).toBe("interrupted");
  });

  it("cancels while binding validation is blocked without spawning", async () => {
    const admitted = deferred<TaskBindingReceipt>();
    let spawned = false;
    const native = runtime();
    const { store, controller } = await fixture(
      {
    ...
    const task = await start(controller);
    await eventually(store, task.id, "starting");
  ...
  it("accepts reordered exact receipts and rejects outside or rebound cwd", async () => {
    ...
    const accepted = await start(controller);
    await eventually(store, accepted.id, "running");
    await expect(
      controller.start({
        parent,
        harness: "native",
        task: "outside",
        binding: { ...binding, cwd: "/elsewhere" },
      }),
    ).rejects.toMatchObject({ code: "invalid_task" });
    let spawned = false;
    const changed = await fixture(
      ...
      {
        async revalidate(receipt) {
          return { ...receipt, generation: receipt.generation + 1 };
        },
      },
    );
    const rejected = await start(changed.controller);
    await eventually(changed.store, rejected.id, "failed");
    expect(spawned).toBe(false);
  });

  it("isolates and freezes binding copies across authority and adapter calls", async () => {
    let authorityFrozen = false;
    let adapterFrozen = false;
    const native = runtime();
    const { store, controller } = await fixture(
      {
        ...native.adapter,
        async start(input, context) {
          adapterFrozen = Object.isFrozen(input.binding);
          return native.adapter.start(input, context);
        },
      },
      {
        async revalidate(receipt) {
          authorityFrozen = Object.isFrozen(receipt);
          expect(Reflect.set(receipt, "cwd", "/mutated")).toBe(false);
          return receipt;
        },
      },
    );
    const task = await start(controller);
    await eventually(store, task.id, "running");
    expect(authorityFrozen).toBe(true);
    expect(adapterFrozen).toBe(true);
    expect((await store.read(task.id)).binding).toEqual(binding);
  });

  it("aborts a blocked handle and tracks it through shutdown", async () => {
    const never = deferred<TaskAdapterHandle>();
    const quiet = deferred<void>();
    const { store, controller } = await fixture({
  ...
  it("lets cancellation enter while follow-up waits on a handle or native acknowledgement", async () => {
    const pendingHandle = deferred<TaskAdapterHandle>();
    const quiet = deferred<void>();
    ...
    const firstTask = await start(first.controller);
    await eventually(first.store, firstTask.id, "running");
    const waitingForHandle = first.controller.followUp(firstTask.id, "more");
    expect((await first.controller.cancel(firstTask.id)).state).toBe("cancelled");
    await expect(waitingForHandle).rejects.toMatchObject({ code: "task_not_running" });

    const followGate = deferred<void>();
    const secondNative = runtime();
    secondNative.adapter.start = async (_input, context) => {
      context.register({
        async force() {
          secondNative.quiet.resolve();
          secondNative.result.resolve("");
        },
        quiescence: secondNative.quiet.promise,
      });
      return {
        result: secondNative.result.promise,
        async followUp() {
          await followGate.promise;
        },
      };
    };
    const second = await fixture(secondNative.adapter);
    const secondTask = await start(second.controller);
    await eventually(second.store, secondTask.id, "running");
    const waitingForFollow = second.controller.followUp(secondTask.id, "more");
    expect((await second.controller.cancel(secondTask.id)).state).toBe("cancelled");
    await expect(waitingForFollow).rejects.toMatchObject({ code: "task_not_running" });
    followGate.resolve();
  });
      ...
      const adapter: TaskAdapter = {
        start(_input, context) {
          ...
          return Promise.resolve({
            result: Promise.reject(new Error("raw result environment")),
            async followUp() {},
          });
        },
      };
      const current = await fixture(adapter);
      const task = await start(current.controller);
      await eventually(current.store, task.id, "running");
      await new Promise((done) => setTimeout(done, 2));
      expect((await current.store.read(task.id)).state).not.toBe("failed");
      expect(forces).toBe(1);
      forceGate.resolve();
      expect((await eventually(current.store, task.id, "failed")).error).toEqual({
        code: "task_failed",
        message: "The native task failed safely.",
      });
    }
  });
  ...
  it("does not publish completion until quiescence and fences late emissions", async () => {
    ...
    await native.context().emit({ code: "late", message: "not recorded" });
    expect((await store.read(task.id)).events).toEqual([]);
  });
  ...
  it("bounds completed results and records truncation", async () => {
    ...
    const task = await start(controller);
    await eventually(store, task.id, "running");
    native.result.resolve("x".repeat(MAX_TASK_RESULT + 10));
    native.quiet.resolve();
    const completed = await eventually(store, task.id, "completed");
    expect(completed.result).toHaveLength(MAX_TASK_RESULT);
  ...
  it("redacts PEM secrets spanning event and result truncation boundaries", async () => {
    ...
    await native.context().emit({ code: "progress", message: eventPem });
    const event = (await store.read(task.id)).events[0]?.message ?? "";
    expect(event).toContain(REDACTED_MEMORY_SECRET);
    expect(event).not.toContain("BEGIN PRIVATE KEY");
    expect(event).not.toContain("SSSSSSSS");
    const resultPem = `${"r".repeat(MAX_TASK_RESULT - 40)}-----BEGIN PRIVATE KEY-----${"K".repeat(200)}-----END PRIVATE KEY-----`;
    native.result.resolve(resultPem);
    native.quiet.resolve();
    const completed = await eventually(store, task.id, "completed");
    expect(completed.result).toContain(REDACTED_MEMORY_SECRET);
    expect(completed.result).not.toContain("BEGIN PRIVATE KEY");
    expect(completed.result).not.toContain("KKKKKKKK");
  });

  it("keeps terminal cancellation byte- and mtime-stable", async () => {
    const native = runtime();
    const { home, store, controller } = await fixture(native.adapter);
    const task = await start(controller);
    await eventually(store, task.id, "running");
    native.result.resolve("done");
    native.quiet.resolve();
    await eventually(store, task.id, "completed");
    const path = join(home, ".tasks", `${task.id}.json`);
    const before = await readFile(path);
    const beforeStat = await stat(path, { bigint: true });
    expect((await controller.cancel(task.id)).state).toBe("completed");
    expect(await readFile(path)).toEqual(before);
    expect((await stat(path, { bigint: true })).mtimeNs).toBe(beforeStat.mtimeNs);
  });

  it("makes old-generation launch and result publication durable no-ops", async () => {
    const validation = deferred<TaskBindingReceipt>();
    let spawned = false;
    const native = runtime();
    const { home, store, controller } = await fixture(
      {
        ...native.adapter,
        async start(input, context) {
          spawned = true;
    ...
    const task = await start(controller);
    await eventually(store, task.id, "starting");
    const recovery = new TaskController(store, new Map(), authority, new FakeNativeTaskScopeManager());
    await recovery.initialize();
    const path = join(home, ".tasks", `${task.id}.json`);
    const before = await readFile(path);
    const beforeStat = await stat(path, { bigint: true });
    validation.resolve(binding);
    await controller.beginShutdown();
    expect(spawned).toBe(false);
    expect(await readFile(path)).toEqual(before);
    expect((await stat(path, { bigint: true })).mtimeNs).toBe(beforeStat.mtimeNs);

    const runningNative = runtime();
    const second = await fixture(runningNative.adapter);
    const active = await start(second.controller);
    await eventually(second.store, active.id, "running");
    const secondRecovery = new TaskController(
      second.store,
      new Map(),
      authority,
      new FakeNativeTaskScopeManager(),
    ...
    const activePath = join(second.home, ".tasks", `${active.id}.json`);
    const activeBefore = await readFile(activePath);
    const activeStat = await stat(activePath, { bigint: true });
    runningNative.result.resolve("late");
    runningNative.quiet.resolve();
    await second.controller.beginShutdown();
    expect(await readFile(activePath)).toEqual(activeBefore);
    expect((await stat(activePath, { bigint: true })).mtimeNs).toBe(activeStat.mtimeNs);
  });
  ...
  it("quiesces a retained native control before reading a terminal record", async () => {
    ...
    const task = await start(current.controller);
    await eventually(current.store, task.id, "running");
    const recovery = new TaskController(
      current.store,
      new Map(),
      authority,
      new FakeNativeTaskScopeManager(),
    ...
    await recovery.initialize();
    expect((await current.controller.cancel(task.id)).state).toBe("interrupted");
    expect(forces).toBe(1);
    result.resolve("late");
    quiet.resolve();
    await current.controller.beginShutdown();
  });
  ...
  it("quiesces an active control exactly once before reporting a read failure", async () => {
    ...
    const store = new FailingReadStore(home);
    stores.push(store);
    ...
    failReads = true;
    let settled = false;
    const cancellation = controller.cancel(task.id);
    void cancellation
      .finally(() => {
        settled = true;
      })
    ...
    await expect(cancellation).rejects.toMatchObject({ code: "task_storage_failed" });
    expect(forces).toBe(1);
  ...
  it("quiesces every live control even when shutdown cannot list storage", async () => {
    ...
    const store = new FailingListStore(home);
    stores.push(store);
    const executions: Array<{ forces: number; quiet: boolean }> = [];
    const controller = new TaskController(
      store,
      new Map([
        [
          ...
          {
            async start(_input, context) {
              const execution = { forces: 0, quiet: false };
    ...
    await eventually(store, second.id, "running");
    failList = true;
    await expect(controller.beginShutdown()).rejects.toMatchObject({
      code: "task_shutdown_failed",
    });
    expect(executions).toEqual([
      { forces: 1, quiet: true },
      { forces: 1, quiet: true },
    ]);
    expect((await store.read(first.id)).state).toBe("interrupted");
    expect((await store.read(second.id)).state).toBe("interrupted");
  });
  ...
  it("quiesces other live work when one admission write fails during shutdown", async () => {
    ...
    class FailingAdmissionStore extends TaskStore {
      override async write(row: TaskRecord): Promise<void> {
        if (rejectAdmission && row.state === "queued") {
    ...
    const store = new FailingAdmissionStore(home);
    stores.push(store);
    const quiet = deferred<void>();
    const result = deferred<string>();
    let forces = 0;
    ...
    await controller.initialize();
    const live = await start(controller);
    await eventually(store, live.id, "running");
    rejectAdmission = true;
    const admission = start(controller).then(
      () => undefined,
      (error: unknown) => error,
    );
    ...
    await expect(shutdown).rejects.toMatchObject({ code: "task_shutdown_failed" });
    expect(forces).toBe(1);
    expect((await store.read(live.id)).state).toBe("interrupted");
  });
  ...
  it("serializes follow-up and repeated cancellation until full quiescence", async () => {
    ...
    const { store, controller } = await fixture(adapter);
    const task = await start(controller);
    await eventually(store, task.id, "running");
    await controller.followUp(task.id, "more");
    const first = controller.cancel(task.id);
    const second = controller.cancel(task.id);
    await new Promise((done) => setTimeout(done, 2));
    expect(order).toEqual(["follow", "force"]);
    gate.resolve();
    quiet.resolve();
    result.resolve("");
    expect((await first).state).toBe("cancelled");
    expect((await second).state).toBe("cancelled");
    expect(order.filter((item) => item === "force")).toHaveLength(1);
  });
  ...
  it("gives shutdown deterministic precedence over cancellation in either ordering", async () => {
    for (const shutdownFirst of [false, true]) {
      ...
      const task = await start(current.controller);
      await eventually(current.store, task.id, "running");
      const first = shutdownFirst
        ? current.controller.beginShutdown()
        : current.controller.cancel(task.id);
      const second = shutdownFirst
        ? current.controller.cancel(task.id)
        : current.controller.beginShutdown();
      gate.resolve();
      if (shutdownFirst) {
  ...
  it("upgrades a late registered cancellation after shutdown begins", async () => {
    ...
    stores.push(store);
    const controller = new TaskController(store, new Map(), authority, new FakeNativeTaskScopeManager());
    await controller.initialize();
  ...
  it("keeps an upgraded shutdown target authoritative across a blocked cancelled write", async () => {
    ...
    const release = deferred<void>();
    let blockCancelled = false;
    class BlockingStore extends TaskStore {
      override async write(row: TaskRecord): Promise<void> {
        if (blockCancelled && row.state === "cancelled") {
          blockCancelled = false;
          blocked.resolve();
          await release.promise;
        }
        await super.write(row);
    ...
    const store = new BlockingStore(home);
    stores.push(store);
    const native = runtime();
    const controller = new TaskController(
      store,
    ...
    const shutdown = controller.beginShutdown();
    release.resolve();
    expect((await cancelled).state).toBe("interrupted");
    await shutdown;
    expect((await store.read(task.id)).state).toBe("interrupted");
  });

  it("strictly advances task, event, and recovery timestamps under a backward clock", async () => {
    let clock = Date.parse("2030-01-01T00:00:10.000Z");
    const now = () => {
      clock -= 1_000;
    ...
    const store = new RecordingStore(home);
    stores.push(store);
    const controller = new TaskController(
      store,
      new Map([["native", native.adapter]]),
    ...
    const task = await start(controller);
    await eventually(store, task.id, "running");
    await native.context().emit({ code: "progress", message: "one" });
    native.result.resolve("done");
    native.quiet.resolve();
    const completed = await eventually(store, task.id, "completed");
    const taskTimes = writes
      .filter((row) => row.id === task.id)
      .map((row) => Date.parse(row.updatedAt));
    expect(taskTimes.every((time, index) => index === 0 || time > taskTimes[index - 1]!)).toBe(
      true,
    );
    expect(
      completed.events.every((event, index) =>
        index === 0
          ? Date.parse(event.at) > Date.parse(completed.createdAt)
          : event.at > completed.events[index - 1]!.at,
      ),
    ).toBe(true);

    const pending = record("running");
    pending.createdAt = "2040-01-01T00:00:00.000Z";
    pending.updatedAt = "2040-01-01T00:00:05.000Z";
    ...
    const recovered = await recovery.initialize();
    expect(recovered.find((row) => row.id === pending.id)?.updatedAt).toBe(
      "2040-01-01T00:00:05.001Z",
    );
  });
  ...
  it("recovers every nonterminal state with a generation fence and skips unrelated names", async () => {
    ...
    expect(recovered.filter((row) => row.state === "interrupted")).toHaveLength(4);
    expect(
      recovered.filter((row) => row.state === "interrupted").every((row) => row.generation === 2),
    ).toBe(true);
    expect(recovered.find((row) => row.id === rows[4]?.id)?.state).toBe("completed");
    expect([...ownership.stops].sort()).toEqual(
      rows
        .slice(0, 4)
        .map((row) => row.id)
        .sort(),
  ...
  it("does not admit a retry when task-store disposal is unconfirmed", async () => {
    ...
    stores.push(store);
    const nativeOwnership = new FakeNativeTaskScopeManager();
  ...
  it("retries scope confirmation after a crash window before recovery publication", async () => {
    ...
    const firstOwnership = new FakeNativeTaskScopeManager();
    ...
    const secondOwnership = new FakeNativeTaskScopeManager();
  ...
  it("keeps monotonic event cursors when bounded history is truncated", async () => {
    ...
    for (let index = 1; index <= 70; index += 1)
      await native.context().emit({ code: "progress", message: String(index) });
    const row = await store.read(task.id);
    expect(row.events).toHaveLength(64);
    expect(row.events[0]?.sequence).toBe(7);
    expect(row.events.at(-1)?.sequence).toBe(70);
  ...
  it("maps adapter errors and redacts bounded arbitrary harness text", async () => {
    ...
    const { store, controller } = await fixture({
      async start(_input, context) {
        context.register({
          async force() {
            throw new Error("Bearer force-secret");
          },
          quiescence: quiet,
        });
        await context.emit({ code: "progress", message: "token=ghp_abcdefghijk" });
        throw new Error("stderr protocol Bearer provider-secret");
      },
    });
    const task = await start(controller);
    const failed = await eventually(store, task.id, "failed");
    expect(JSON.stringify(failed)).not.toContain("ghp_abcdefghijk");
  ...
  it("rejects unsafe task directories and record links or modes", async () => {
    ...
    const task = await start(controller);
    await eventually(store, task.id, "running");
    const file = join(home, ".tasks", `${task.id}.json`);
    await link(file, join(home, ".tasks", "extra-link"));
    await expect(store.read(task.id)).rejects.toMatchObject({ code: "unsafe_task_record" });
    const secondHome = await mkdtemp(join(tmpdir(), "ghost-task-"));
    await symlink(tmpdir(), join(secondHome, ".tasks"));
    await expect(trackedStore(secondHome).initialize()).rejects.toBeTruthy();
  });
  ...
  it("rejects directory replacement and keeps durable files exactly private", async () => {
    ...
    expect((await stat(join(home, ".tasks"))).mode & 0o777).toBe(0o700);
    await rename(join(home, ".tasks"), join(home, ".tasks-old"));
    await mkdir(join(home, ".tasks"), { mode: 0o700 });
    await expect(store.read(task.id)).rejects.toMatchObject({ code: "unsafe_task_store" });
  });
  ...
  it("recovers a durable CAS crash and rejects an inode replacement race", async () => {
    ...
    const store = trackedStore(home);
    await store.initialize();
    const row = record("queued");
    await store.write(row);
    const path = join(home, ".tasks", `${row.id}.json`);
    await rename(path, `${path}.ghost-migration-cas`);
    expect((await store.list())[0]?.id).toBe(row.id);
    const admitted = await store.read(row.id);
    const replacement = `${path}.replacement`;
    await writeFile(replacement, `${JSON.stringify(admitted, null, 2)}\n`, { mode: 0o600 });
    await rename(replacement, path);
    admitted.state = "starting";
    await expect(store.write(admitted)).rejects.toMatchObject({ code: "private_write_conflict" });
  ...
  it("fails closed on exact schema, invariant, and timestamp violations", async () => {
    ...
    await store.write(row);
    const path = join(home, ".tasks", `${row.id}.json`);
    await writeFile(path, JSON.stringify({ ...row, createdAt: "yesterday", unknown: true }), {
      mode: 0o600,
    });
    ...
    const badCursor = {
      ...row,
      events: [{ sequence: 2, at, code: "progress", message: "gap" }],
      eventCursor: { nextSequence: 3, dropped: 0 },
    };
    await writeFile(path, JSON.stringify(badCursor), { mode: 0o600 });
    await expect(store.read(row.id)).rejects.toMatchObject({ code: "invalid_task_record" });
    const firstAt = new Date(Date.parse(at) + 2_000).toISOString();
    const secondAt = new Date(Date.parse(at) + 1_000).toISOString();
    const reversed = {
      ...row,
      updatedAt: firstAt,
      events: [
        { sequence: 1, at: firstAt, code: "progress", message: "first" },
        { sequence: 2, at: secondAt, code: "progress", message: "second" },
      ],
      eventCursor: { nextSequence: 3, dropped: 0 },
    };
    await writeFile(path, JSON.stringify(reversed), { mode: 0o600 });
    await expect(store.read(row.id)).rejects.toMatchObject({ code: "invalid_task_record" });
    await writeFile(path, JSON.stringify({ ...row, task: " \n\t " }), { mode: 0o600 });
    await expect(store.read(row.id)).rejects.toMatchObject({ code: "invalid_task_record" });
    await writeFile(path, JSON.stringify({ ...row, agent: "claude-only" }), { mode: 0o600 });
    await expect(store.read(row.id)).rejects.toMatchObject({ code: "invalid_task_record" });
    await writeFile(
      path,
      JSON.stringify({ ...row, harness: "claude-code", agent: "token=private-value" }),
      { mode: 0o600 },
    );
    await expect(store.read(row.id)).rejects.toMatchObject({ code: "invalid_task_record" });
