...
import type { SettledConversationTurn } from "../src/settled-conversation-turn.js";
...
function turn(ordinal: number, text = `answer-${ordinal}`): SettledConversationTurn {
  return {
...
describe("presentation history", () => {
  it("atomically records, reads, and deduplicates one settled native turn", async () => {
    ...
    const state = await store.recordSettledTurn(dir, identity, turn(1), fresh);
    ...
    await expect(store.recordSettledTurn(dir, identity, turn(1), fresh)).resolves.toEqual(state);
  ...
  it("serializes concurrent writes and marks a missing native ordinal", async () => {
    ...
    await Promise.all([
      store.recordSettledTurn(dir, identity, turn(1), fresh),
      store.recordSettledTurn(dir, identity, turn(3), fresh),
    ]);
  ...
  it("rejects stale ordinals and an equal ordinal with a different revision", async () => {
    const dir = sessionDir();
    const store = new PresentationHistoryStore();
    await store.recordSettledTurn(dir, identity, turn(2), fresh);
    await expect(store.recordSettledTurn(dir, identity, turn(1), fresh))
    ...
    }, fresh)).rejects.toMatchObject({ code: "presentation_history_invalid", status: 500 });
  });
  ...
  it("requires a Claude revision to equal its durable owner ordinal", async () => {
    ...
    const claudeTurn: SettledConversationTurn = {
      source: {
        runtime: "claude-code",
    ...
    await expect(store.recordSettledTurn(dir, claudeIdentity, claudeTurn, fresh))
      .rejects.toMatchObject({ code: "presentation_history_invalid", status: 500 });
  });
  ...
  it("fails closed for mismatched identity, malformed sequence, permissions, and hard links", async () => {
    ...
    for (const mutate of variants) {
      const dir = sessionDir();
      const store = new PresentationHistoryStore();
      const valid = await store.recordSettledTurn(dir, identity, turn(1), fresh);
      const path = presentationHistoryPath(dir, "pi", identity.conversationId);
    ...
    await permissionStore.recordSettledTurn(permissionDir, identity, turn(1), fresh);
  ...
  it("does not let a presentation file publish or alias a legacy Pi identity", async () => {
    ...
    await store.recordSettledTurn(dir, unsafe, turn(1), fresh);
