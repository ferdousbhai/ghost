...
`ghost login <provider>` runs the same login the HUD's model switcher
...
login, which the daemon times out on its own. `ghost login --list` lists the
providers and their signed-in accounts, and `ghost logout <provider>` removes
one. `ghostd login` is the offline equivalent for a machine whose
